...
export default function AuthModal({ onClose, onLoginSuccess }: AuthModalProps) {
  ...
  const handleSocialSimulate = (provider: string) => {
    resetMessages();
    setLoading(true);
    // Simulate secure Social OAuth Authentication callback
    setTimeout(async () => {
      try {
        const dummyEmail = `${provider.toLowerCase().replace(/\s/g, '')}@example-oauth.com`;
        const res = await fetch('/api/auth/register', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: dummyEmail, password: 'SecureSocialOauthPass99!', name: `Social Explorer (${provider})` })
        });
        const regData = await res.json();
        
        // Social skips verification
        const loginRes = await fetch('/api/auth/login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: dummyEmail, password: 'SecureSocialOauthPass99!' })
        });
        const loginData = await loginRes.json();
        if (!loginRes.ok) throw new Error(loginData.error);

        onLoginSuccess(loginData.user, loginData.token);
        setSuccess(`Logged in securely via verified ${provider} identity.`);
      ...
      <div className="relative w-full max-w-md bg-[#FDFCFB] border border-[#E5E5E5] shadow-2xl rounded-none py-10 px-8 transition-all overflow-hidden">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[12px] text-[#6B705C] font-semibold italic mt-2.5">
            ...
            {mode === 'register' && 'Create your aura identity to unlock 50 complimentary points'}
