...
export default function CartDrawer({
  ...
  // Redeem 100 points = $5. Max 50% discount value.
  ...
  if (redeemPoints && user) {
    ...
    const maxRedeemablePoints = Math.floor(maxDiscountAllowed / 5) * 100;
    pointsToRedeem = Math.min(pointsUserHas, maxRedeemablePoints);
    pointsDiscountValue = (pointsToRedeem / 100) * 5;
  }
  ...
  const checkPromoCode = async () => {
    ...
    try {
      ...
      if (data.minSpend && subtotal < data.minSpend) {
        throw new Error(`Minimum spend of $${data.minSpend} required to utilize this coupon.`);
      }
      ...
      <div className="p-6 border-b border-[#E5E5E5] flex items-center justify-between bg-[#FDFCFB]">
        <div className="flex items-center space-x-2.5">
          ...
          <h2 className="font-serif text-[15px] font-medium uppercase tracking-[0.2em] text-[#1A1A1A]">
            {checkoutStep === 'cart' && 'Your Apothecary Bag'}
            {checkoutStep === 'shipping' && 'Sanctuary Delivery'}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 12 Botanical Boulevard"
            ...
            {!pushSent ? (
              <div className="space-y-4">
                ...
                <div className="p-3 bg-[#F5F2ED] border border-[#E5E5E5] text-center font-mono text-[11px] font-semibold text-stone-700 rounded-none">
                  Total Chargeable Balance: <span className="font-bold text-[#1A1A1A]">${finalTotal.toFixed(2)}</span>
                </div>
                  ...
                  <div className="space-y-4 py-2">
                    ...
                    <div className="bg-white text-black p-4 rounded-xl shadow-lg space-y-3 text-center">
                      ...
                      <div className="space-y-1 block">
                        ...
                        <p className="text-xs font-bold text-[#1A1A1A]">Pay LATAM NATURAL</p>
                        <p className="text-lg font-serif font-bold text-red-600">${finalTotal.toFixed(2)}</p>
                      </div>
          ...
          <div className="space-y-2.5 text-xs pb-2 border-b border-neutral-100 bg-[#FDFCFB]">
            ...
            {discountAmount > 0 && (
              <div className="flex justify-between text-emerald-800 font-semibold">
                <span>Sanctuary Discount ({activePromo?.code})</span>
            ...
            {pointsToRedeem > 0 && (
              <div className="flex justify-between text-[#6B705C] font-semibold">
                <span>Royalty Point Offset ({pointsToRedeem} pts)</span>
